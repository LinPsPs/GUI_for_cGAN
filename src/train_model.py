...
def load_data(batch_size=1, is_val=False):
        ...
        path = glob('../input/anime-sketch-colorization-pair/data/%s/*' % (data_type))
...
train(epochs=100, batch_size=32, show_interval=10)
real_time = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
generator.save('/kaggle/working/' + str(real_time) + '.h5')
